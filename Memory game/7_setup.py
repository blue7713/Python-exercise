...
def setup(level):
    ...
    display_time = max(display_time, 1) # 최소 1초 보여주기

    # 얼마나 많은 숫자를 보여줄 것인가?
    number_count = (level // 3) + 5
    number_count = min(number_count, 20) # 최대 카운트 수 20

    # 실제 화면에 그리드 형태로 숫자를 랜덤으로 배치
...
def shuffle_grid(number_count):
    ...
    # 시작 숫자를 1부터 number_count 까지,
    # 만약 5라면 5까지 숫자를 랜덤으로 배치
    number = 1 
    while number <= number_count:
        ...
        if grid[row_idx][col_idx] == 0:
            ...
            # 현재 grid cell 위치 가준으로 x, y 위치를 구함
...
def display_game_screen():
    ...
    for idx, rect in enumerate(number_buttons, start = 1): # 인덱스를 1부터 시작, number_button을 rect에 삽입
        if hidden == True: # 숨김 처리
            # 버튼 사각형 그리기
            pygame.draw.rect(screen, WHITE, rect)
        else:
            # 실제 숫자 텍스트
            cell_text = game_font.render(str(idx), True, WHITE) # idx값을 흰색으로 
            text_rect = cell_text.get_rect(center = rect.center) # text를 기준으로 그린 rect의 center값
            screen.blit(cell_text, text_rect) # cell_text 그리기, number_buttons의 center

# pos 에 해당하는 버튼 확인
def check_buttons(pos):
    global start, start_ticks

    if start == True: # 게임이 시작했으면?
        check_number_buttons(pos)
    elif start_button.collidepoint(pos):
        start = True
        start_ticks = pygame.time.get_ticks() # 타이머 시작 (현재 시각을 저장)
